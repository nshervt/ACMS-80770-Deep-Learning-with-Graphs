...
def su22vtk(mesh_filename, vtk_filename, write = True):
    nodes, edges, elems, marker_dict = get_mesh_graph(mesh_filename)
    coords = np.hstack((nodes,np.repeat(0.,len(nodes))[:,np.newaxis]))
    tris = np.hstack((np.repeat(3,len(elems[0]))[:,np.newaxis],np.array(elems[0])))
    # print(type(nodes),type(edges),type(elems))
    print(np.array(elems[0]))
    # print(nodes.shape)
    print(nodes.shape, edges.shape, tris.shape)
    new_mesh = pv.PolyData(coords, tris)
    if write == True:
        new_mesh.save(vtk_filename)
    return new_mesh
...
def get_pathlength(line):
    segs = np.linalg.norm((line[1:] - line[:-1]), axis = 1)
    abscissa = np.cumsum(segs)/sum(segs)
    return np.insert(abscissa,0,0.0)

def cl_dist_graph(nodes, clpc_name = './data/cl_spline.npy', with_sign=False, is2d =True):
    # assumes shape is convex
    # approximate signed distance by distance to closest point on surface
    clpc = np.load(clpc_name)
    if is2d == True and clpc.shape[1] == 3:
        clpc = clpc[:,:2]        
    radial_dists = np.zeros(nodes.shape[0])
    stream_dists = np.zeros(nodes.shape[0])
    abscissa = get_pathlength(clpc)
    for i, x in enumerate(nodes):
        vecs = clpc - x
        dists = np.linalg.norm(vecs,axis = 1)
        min_dist = np.min(dists)
        min_index = np.argmin(dists)
        radial_dists[i] = min_dist
        stream_dists[i] = abscissa[min_index]
    radial_dists, stream_dists = np.array(radial_dists), np.array(stream_dists)
    radial_dists = radial_dists/np.max(radial_dists)
    return radial_dists, stream_dists
