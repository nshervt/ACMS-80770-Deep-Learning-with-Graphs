...
def run_forward(comm: MPI.Intracomm, forward_driver: pysu2.CSinglezoneDriver,
    ...
    """
    rank = comm.Get_rank()
    # ts = inputs[-1]  # this the the current time step 
    
    # # add for extra input
    # inputs = inputs[:-1]
    
    # print('pan debug su2_function_mpi, calling run forward, print inputs shape:',len(inputs))
    # print('pan debug su2_function_mpi, calling run forward, print forward time step:',ts)

    for i, x in enumerate(inputs):  # here the inputs should only include coordx and coordy 
        # print('run_forward inputs size and value',x.shape, x)
...
def run_adjoint(comm: MPI.Intracomm, adjoint_driver: pysu2ad.CDiscAdjSinglezoneDriver,
    ...
    print('pan debug su2_function_mpi, calling run adjoint, print inputs shape:',len(inputs))
...
def activate_su2_mpi(remove_temp_files: bool = True, max_procs_per_example: int = 1,
    ...
    import su2_function_mdf
    
    su2_function_mdf._global_max_ppe = max_procs_per_example
...
def main(remove_temp_files: bool = True) -> None:
    ...
    while True:
        ...
        if run_type == RunCode.RUN_FORWARD:
            ...
            # here the inputs include {coordx, coordy, ts}
            batch_size = inputs[0].shape[0]
            # ts = inputs[-1]  # get the current time step 
            # print('pan debug in su2_function_mpi: print local rank, procs-per example', local_rank, procs_per_example)
            print('pan debug in su2_function_mpi: print local rank, inputs size', local_rank, len(inputs))
            ...
            # add for extra input 
            x = [z[batch_index] for z in inputs[:-1]]  # here we have x is coordx and cooordy
            # x = [z[batch_index] for z in inputs]
            input_profile_index = int(inputs[-1][batch_index])
            input_profile_filename = './data/inlet_3/inet_{:d}.dat'.format(input_profile_index)
            ...
            if batch_rank == 0:
                ...
                new_config = {'RESTART_FLOW_FILENAME': batch_restart_filename,
                              ...
                              'INLET_FILENAME': input_profile_filename
                              }
            ...
            outputs = run_forward(batch_comm, forward_driver, x) # x: this is only coordx and coordy 
            ...
            print('node grads len', len(grads))
            print('grads 0 size and value', grads[0].shape, grads[0])
            print('grads 1 size and value', grads[1].shape, grads[1])
            non_busy_post(MPI.COMM_WORLD)
